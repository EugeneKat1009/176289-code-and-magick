...
var getColorElement = function (name) {
  return name === 'Вы' ? 'red' : 'rgba(0, 0, 255, ' + (Math.random() * (1 - 0.3) + 0.3) + ')';
};
...
window.renderStatistics = function (ctx, names, times) {
  ...
  for (var i = 0; i < players.length; i++) {
    ...
    ctx.fillText(Math.floor(times[i]), CLOUD_X + GAP * 2 + (BAR_WIDTH + BAR_GAP) * i, CLOUD_HEIGHT - barHeight - GAP - FONT_HEIGHT - GAP);

    ctx.fillStyle = getColorElement(players[i]);
    ctx.fillRect(CLOUD_X + GAP * 2 + (BAR_WIDTH + BAR_GAP) * i, TITLE_HEIGHT + (BAR_HEIGHT - barHeight) + GAP + GAP, BAR_WIDTH, barHeight);
  }
};
